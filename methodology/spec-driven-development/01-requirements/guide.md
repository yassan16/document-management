...
チーム開発では、**メンバー間の認識を揃える** ために、正常系・エラーケース・優先度を明確にします。テンプレートの全セクションを記載し、非同期レビューで合意をとります。Medium規模の完成形は [具体例: Todoアプリの requirements.md](#具体例-todo-アプリの-requirementsmd) を参照してください。
...
> READMEでは Gate1 の確認事項を「ゴールが明確か？スコープ外が定義済みか？ユーザーストーリーに漏れはないか？」と定義しています。このガイドでは、その3つの観点に「技術仕様の混入チェック」を加え、より詳細な判断フローを提供します。
...
> [02-design/guide.md — 「どう作るか」を設計する](../02-design/guide.md)

# Methodology

## 概要

開発手法・プロセスに関するドキュメントを整理しています。アジャイル開発、バグ対応、バッチ処理設計、API開発、コードリーディングなどを扱います。

---

## 目次

| カテゴリ | 内容 |
|---------|------|
| [Agile](./agile/) | スクラム知識、準備、トレーニング |
| [API Development](./api-development/) | API開発ロードマップ |
| [Batch](./batch/) | バッチ処理設計（中間データ） |
| [Code Reading](./code-reading/) | コードリーディング学習ロードマップ |
| [バグ対応プロセス](./バグ対応プロセス.md) | 不具合の受付から原因調査、修正、検証、リリース後確認までの標準プロセス |
| [仕様書駆動開発](./仕様書駆動開発/概要.md) | 仕様書駆動開発 |
| [テスト駆動開発](./テスト駆動開発/概要.md) | テスト駆動開発 |
| [要件定義](./要件定義/要件定義の進め方.md) | 一般的な要件定義の進め方 |

---

## ディレクトリ構造

```
04-開発手法/
├── agile/                       # アジャイル開発
│   ├── scrum-knowledge.md       # スクラム知識
│   ├── scrum-preparation.md     # スクラム準備
│   └── scrum-training.md        # スクラムトレーニング
├── api-development/             # API開発
│   └── roadmap.md               # 学習ロードマップ
├── batch/                       # バッチ処理
│   └── transitional-data.md     # 中間データ設計
├── code-reading/                # コードリーディング
│   └── roadmap.md               # 学習ロードマップ
├── バグ対応プロセス.md            # バグ対応の標準プロセス
├── 仕様書駆動開発/
│   ├── 概要.md
│   ├── 01-要件定義/
│   │   ├── ガイド.md
│   │   └── テンプレート.md
│   ├── 02-設計/
│   │   ├── ガイド.md
│   │   └── テンプレート.md
│   └── 03-タスク分割/
│       ├── ガイド.md
│       └── テンプレート.md
├── テスト駆動開発/
│   └── 概要.md                  # TDD概要・実践ガイド
└── 要件定義/                     # 要件定義
    └── 要件定義の進め方.md       # 一般的な要件定義の進め方
```

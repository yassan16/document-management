...
サブエージェントは、Claude Code 内で特定のタスクを処理する専門化された AI アシスタントです。各サブエージェントは独自のコンテキストウィンドウで動作し、ツールアクセスや権限を個別に制御できます。

### 組み込みサブエージェント

| サブエージェント | モデル | 用途 |
|----------------|--------|------|
| **Explore** | Haiku | ファイル検索、コード探索（読み取り専用） |
| **Plan** | 継承 | プランモード中のリサーチ（読み取り専用） |
| **General-purpose** | 継承 | 複雑なマルチステップ操作 |

### カスタムサブエージェント

YAML frontmatter 付きの Markdown ファイルで定義し、`.claude/agents/`（プロジェクト）や `~/.claude/agents/`（ユーザー）に配置します。`/agents` コマンドで対話的に作成・管理も可能です。
...
|------|------|
| 対話的な作業 | メイン会話 |
| 大量出力の分離・ツール制限 | サブエージェント |
| 再利用可能なプロンプト | Skills |

> **詳細は → [04-sub-agents.md](./04-sub-agents.md)**（組み込み/カスタムの全種類、設定フィールド、ツール・権限制御、永続メモリ、Hooks連携、実践例など）
...
| `.claude/agents/*.md` | する | チーム共有のサブエージェント |
...
- [サブエージェント詳細ガイド](./04-sub-agents.md) - 組み込み/カスタムサブエージェントの設定・活用ガイド
- [MCP（Model Context Protocol）ガイド](../mcp/README.md) - MCP の概念・設定方法・セキュリティの体系的ガイド

...
2. [マイグレーション](#2-マイグレーション)
   ...
   - [2.9 統合マイグレーション（schema:dump）](#29-統合マイグレーションschemadump)
3. [シーダー](#3-シーダー)
...
### 2.9 統合マイグレーション（schema:dump）

プロジェクトが成長すると、マイグレーションファイルが数十〜数百に膨れ上がります。**統合マイグレーション（Schema Squashing）** は、これらのマイグレーションファイルを **1つのSQLスキーマダンプファイルに統合** する仕組みです。`php artisan schema:dump` コマンドで実行し、`database/schema/` ディレクトリにSQLファイルを生成します。

**なぜ統合マイグレーションが必要なのか？**

- **マイグレーションファイルの肥大化防止** — ファイル数が増えるとディレクトリが見づらくなり、管理コストが上がる
- **`migrate:fresh` の大幅な高速化** — 個別のマイグレーションを順次実行する代わりに、SQLダンプを一括ロードするため処理が高速化する（実測例: 11秒 → 1.4秒、約90%短縮）
- **新メンバーやCI/CDのセットアップ効率化** — 環境構築時にゼロからマイグレーションを全件実行する必要がなくなる
...
| オプション     | 説明                                       | 使用例                                        |
| -------------- | ------------------------------------------ | --------------------------------------------- |
| `--database`   | ダンプ対象のDB接続名を指定                 | `schema:dump --database=pgsql`                |
| `--path`       | スキーマダンプの出力先パスを指定           | `schema:dump --path=database/schema/custom`   |
| `--prune`      | ダンプ後に既存のマイグレーションファイルを削除 | `schema:dump --prune`                         |

```bash
# 基本的な使い方（デフォルト接続でダンプ）
php artisan schema:dump

# PostgreSQL接続を指定してダンプ
php artisan schema:dump --database=pgsql

# ダンプ後にマイグレーションファイルを削除
php artisan schema:dump --prune
```

#### 仕組みとディレクトリ構造

`schema:dump` を実行すると、`database/schema/` ディレクトリに接続名に基づいたSQLファイルが生成されます。ファイル名は `{connection}-schema.sql` の規則に従います。

```
database/
├── migrations/
│   ├── 2025_07_01_000001_create_users_table.php      # --prune で削除される
│   ├── 2025_07_01_000002_create_posts_table.php       # --prune で削除される
│   ├── 2025_07_01_000003_create_tags_table.php        # --prune で削除される
│   ├── 2025_07_01_000004_create_comments_table.php    # --prune で削除される
│   └── 2026_03_01_000001_add_avatar_to_users.php      # ダンプ後に追加された新規マイグレーション
├── schema/
│   └── mysql-schema.sql                               # schema:dump で生成されたSQLダンプ
├── factories/
└── seeders/
```

生成されるSQLファイルには、テーブル定義だけでなく `migrations` テーブルの記録（どのマイグレーションが実行済みか）も含まれます。

#### マイグレーション実行フロー

`php artisan migrate` を実行した際、スキーマダンプが存在する場合のフローは以下の通りです。

```mermaid
flowchart TD
    A[php artisan migrate 実行] --> B{migrationsテーブルが空か？}
    B -- Yes --> C{schema/*.sql が存在するか？}
    B -- No --> F[未実行のマイグレーションを順次実行]
    C -- Yes --> D[スキーマSQLを一括実行]
    C -- No --> E[全マイグレーションを順次実行]
    D --> F
    F --> G[完了]
    E --> G
```
...
**ポイント:**
- スキーマダンプは **`migrations` テーブルが空の場合にのみ** 使用される（＝初回セットアップ時やテスト時）
- `migrate:fresh` は全テーブルをドロップした後、上記フローを実行するため、スキーマダンプの恩恵を直接受ける
- テストで `RefreshDatabase` トレイトを使用している場合も同様に高速化される

#### 推奨ワークフロー

統合マイグレーションを安全に導入するためのステップバイステップ手順です。
...
**ステップ2: 生成されたSQLファイルを確認する**

```bash
# 生成されたファイルを確認
cat database/schema/mysql-schema.sql

# テーブル定義が正しいことを確認（usersテーブルの例）
# CREATE TABLE `users` (
...
git commit -m "chore: add schema dump for migration squashing"
```

**ステップ4:（オプション）古いマイグレーションファイルを削除する**

```bash
# 必ず前のステップでコミットしてから実行すること
php artisan schema:dump --prune
git add -A
git commit -m "chore: prune old migration files after schema dump"
```

**ステップ5: 動作確認**

```bash
# データベースを再作成して正常に動作するか確認
...
| 使うべき場面                                         | 使わないべき場面                                   |
| ---------------------------------------------------- | -------------------------------------------------- |
| マイグレーションファイルが50以上に増えた              | プロジェクト初期（マイグレーションが少ない）        |
| `migrate:fresh` の実行時間が長くなってきた            | マイグレーション履歴を厳密に追跡する必要がある場合  |
| CI/CDパイプラインのテスト実行を高速化したい           | 複数DB接続を使っていてスキーマが複雑な場合          |
| 新メンバーのローカル環境構築を効率化したい            | SQL Serverを使用している場合（非対応）              |
| 6〜12ヶ月ごとの定期的なメンテナンスとして             |                                                    |

**頻度の目安:** 6〜12ヶ月ごと、またはマイグレーションファイルが50を超えたタイミングで実施を検討してください。
...
SQL Serverにはネイティブなスキーマダンプ機能（`mysqldump` や `pg_dump` に相当するもの）がLaravelから直接利用できる形で存在しないため、現時点では非対応となっています。

#### 注意点

- **本番環境ではスキーマファイルは無視される** — スキーマダンプは `migrations` テーブルが空の場合にのみ実行される。本番環境ではすでにマイグレーションが適用済みのため、スキーマファイルは使われない
- **`--prune` は不可逆操作** — 削除されたマイグレーションファイルは復元できないため、必ず `schema:dump` の結果をコミットしてから `--prune` を実行すること
- **チーム開発時の運用ルール** — スキーマダンプを更新した場合、全メンバーがスキーマファイルを `git pull` してから新しいマイグレーションの作成・実行を行うこと。古いスキーマファイルのまま作業すると、環境間でDBの状態に差異が生じる可能性がある
- **スキーマダンプは上書き生成される** — `schema:dump` を再実行すると、既存のスキーマファイルは上書きされる。前回のダンプとの差分を確認したい場合は、事前にコミットしておくこと
...
- [Laravel 12.x - Squashing Migrations](https://laravel.com/docs/12.x/migrations#squashing-migrations) — 統合マイグレーション（schema:dump）公式ドキュメント
- [Laravel News: Schema Dump](https://laravel-news.com/schema-dump) — schema:dump の解説記事
